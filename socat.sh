...
start_socat(){
    ...
    nohup socat -T 600 UDP6-LISTEN:${port1},reuseaddr,fork UDP6:${socatip}:${port2} >> /root/socat.log 2>&1 &
    if [ "${OS}" == 'CentOS' ];then
        sed -i '/exit/d' /etc/rc.d/rc.local
        echo "nohup socat TCP6-LISTEN:${port1},reuseaddr,fork TCP6:${socatip}:${port2} >> /root/socat.log 2>&1 &
        nohup socat -T 600 UDP6-LISTEN:${port1},reuseaddr,fork UDP6:${socatip}:${port2}  >> /root/socat.log 2>&1 &
        " >> /etc/rc.d/rc.local
